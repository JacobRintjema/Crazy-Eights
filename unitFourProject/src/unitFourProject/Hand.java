 ...
 * September 2019
 * Crazy 8 Project
 * This class makes the player's hand;
 */
...
public class Hand extends Deck {//class definition to construct the object; inherits methods from deck
	...
	boolean hasCard(Card card) {//object's copy of the hasCard method
		...
		for (int i = 0; i < this.size(); i++) {//iterates according to the number of cards in the hand
			if (this.get(i).getValue() == card.getValue() || this.get(i).getSuit().equals(card.getSuit())) {//checks if card in hand has same value property as card
	...
	Pile playCard(Pile pile, Card card, String suit, int value) {//object's copy of the playCard method
