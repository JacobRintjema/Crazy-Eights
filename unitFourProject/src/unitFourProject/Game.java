 ...
 * September 2019
 * Crazy 8 Project
 * This class makes the methods and variables used for the game;
 */
...
public class Game {//class definition to construct the object
	...
	public static void logo() {//object's copy of the logo method
		...
		System.out.println("");//print empty line
		System.out.println("		By: Jacob Rintjema");//print author
	}
	...
	void playerPickup(Card card) {//object's copy of the playerPickup method
		...
		if (card.getSuit().equals(pile.getTopCard().getSuit()) || card.getValue() == pile.getTopCard().getValue()) {
			...
			if (playerOption.charAt(0) == 'y') {//checks if first character is a y
				pile = players.getCurrentPlayer().getHand().playCard(pile, pile.getTopCard(), card.getSuit(), card.getValue());//returns a new pile by calling the playCard method on the current player's hand
			}
		}
	}
	...
	boolean deckCheck() {//object's copy of the deckCheck method
		...
		while(deck.get(0).getValue() == 8) { //Check to see if starting card on pile is an 8
			deck.shuffle();//call shuffle method on deck
		}
	...
	void playerTurns() {//object's copy of the playerTurns method
		...
		while (loopThru) {
			...
			if (playerOption.equals("1")) {//checks if playerOption reference equals to 1 in value
				...
				this.clearScreen();//call clearScreen method on game
				if (players.getCurrentPlayer().getHand().hasCard(pile.getTopCard())) {//checks if the hasCard method return true by calling it on current player's hand
				...
				pile.print();//call the print method on pile
				players.getCurrentPlayer().getHand().print();//call the print method on the current player's hand
				pile = players.getCurrentPlayer().getHand().playCard(pile, pile.getTopCard(), temp[0], Integer.parseInt(temp[1]));//returns a new pile by calling the playCard method on the current player's hand
				if ((tempCard.getValue() != pile.getTopCard().getValue() || !(tempCard.getSuit().equals(pile.getTopCard().getSuit())))) {//checks if the player has played a card yet by checking if the top card changed
					loopThru = false;
				}
				...
				if (!deckCheck()) {//checks if player has not moved
	...
	void playerWin() {//object's copy of the playerWin method
		...
		if (players.size() > 1) {//checks if there are more than one player
			System.out.println();//empty line
			System.out.println("Would you like to play again? (Yes/No)");//asks if the users wants to play again
			playerOption = scan.nextLine().trim().toLowerCase();//references to a string
